...
---
- <span style="color: blue">栈(stack): 栈(stack)为自动分配的内存空间，它由系统自动释放(**内存中间区域**)</span>

    所谓栈内存,其实就是在计算机内存中分配出来的一块空间。用于提供浏览器执行代码的执行环境以及存储基本类型值
    
- <span style="color: blue">堆(heap): 堆(heap)是动态分配的内存,大小不定也不会自动释放(**内存**)</span>

    用于存储引用类型值

<span style="color: red">**栈为执行线程留出的临时空间的内存，堆为动态分配预留的内存，文件一般在硬盘盘中**</span>
...
如下图所示:<span style="color: blue">栈内存中存放的只是该对象的访问地址，在堆内存中为这个值分配空间。由于这种值的大小不固定，因此不能把他们保存到栈内存中。但是内存地址大小的固定的，因此可以将内存地址保存在栈内存中。这样当查询引用类型的变量时，先从栈中读取内存地址，然后在通过地址找到堆中的值。</span>
...
<span style="color: blue">为了使程序运行时占用的内存最小，通常要实现垃圾回收机制</span>

<span style="color: blue">**当一个方法执行时，每个方法都会建立自己的内存栈，在这个方法内定义的变量将会逐个放入这块栈内存里，随着方法的执行结束，这个方法的栈存也将自然销毁了。因此，所有在方法中定义的变量都是放在栈内存中的**</span>
...
## 资料
[栈内存 & 堆内存 (数据类型间的存储区别)](https://blog.csdn.net/Mirror_r/article/details/116651529)
...
- <span style="color: red">在对象结束使用后，令obj = null.这样利于解除循环引用，使得无用变量及时被回收</span>
