...
首先，面板中会有很多的Task，如果是耗时长的Task,其右上角会标红(图中灭有，说明页面首屏的逻辑处理分配的还不错)，这个时候，我们可以选中表中的task,然后放大(选中，滑动鼠标可放大)，看其具体的耗时点
