...
1. 如果我们需要一个只跟着视图走的数据，我们不能直接使用props或者state.这个我们可以通过一个实例看看
    ...
    **加入我一开是传入的props值是小红，然后三秒之内去改变props.user的值，变成小绿，这两个分别输出小绿和小红**。为什么会这样呢？这里我先只介绍类组件，在React的类组件中，props虽然是不变的，但是this永远是可变的。当有异步的事件触发，它获取到的props或者state永远都是最新的
2. 使用bind或者剪头函数去约束我们函数中this的作用域
...
### 高阶组件
定义：高阶组件其实就是高阶函数，我们顶一个函数里面返回一个有状态组件，高阶组件的好处就是让我们的业务逻辑层和UI层分离，更加好维护
